...
<template>
  <el-popover :visible="isShow" placement="bottom" :width="200" popper-class="!px-0 !py-1 ignore ">
    <div ref="myPopoverRef">
      ...
      <el-divider class="my-2" v-if="cacheList.length"/>
      <div class="op-title">
        {{ optionsTitle }}
      </div>
      <div class="max-h-[200px] overflow-y-auto">
    ...
    <template #reference>
      <div @click.stop="onPopoverShow" :class="`bg-white flex text-font2 w-[170px] items-center px-2 mr-4 border py-[5px] rounded-[4px] hover:( border-[#ccc]) ${isShow ? 'border-primary' : ''}`">
