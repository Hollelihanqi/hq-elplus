...
const outputDir: string = path.resolve(
  __dirname,
  '../yto-custom/src/components',
  compName
)
const mainFilePath: string = path.join(
  'packages/yto-custom/src/components',
  compName,
  `${compName}.vue`
)
...
async function generate (): Promise<[void, void]> {
  updatedFiles.push(
    `packages/yto-custom/src/components/${compName}/**`,
    'packages/yto-custom/src/index.ts',
  )
...
async function updateComponentEntry (): Promise<void> {
  const entryFilePath: string = path.resolve(
    __dirname,
    '../yto-custom/src/components/index.ts'
  )
