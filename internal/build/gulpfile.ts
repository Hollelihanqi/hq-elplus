...
import { buildOutput, epOutput, epPackage, projRoot, epRoot } from "@yto-uplus/build-utils";
...
export const copyFiles = () =>
  Promise.all([
    ...
    copyFile(path.resolve(epRoot, "README.md"), path.resolve(epOutput, "README.md")),
    // copyFile(path.resolve(projRoot, "global.d.ts"), path.resolve(epOutput, "global.d.ts")),
  ]);

export const copyTypesDefinitions: TaskFunction = (done) => {
  const src = path.resolve(buildOutput, "types", "packages");
  console.log("srcbuildOutput__________", src);

  // const copyTypes = (module: Module) => {
  //   console.log("module__________", module);
  //   return withTaskName(`copyTypes:${module}`, () => copy(src, buildConfig[module].output.path, { recursive: true }));
  // };
  const copyTypes = (module: Module) =>
